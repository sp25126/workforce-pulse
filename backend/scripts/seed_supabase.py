...
def seed_database():
    ...
    # Split schema SQL statements by semicolon and run them
    # Note: simple parser, works well for standard schema setups
    with engine.begin() as conn:
        # Split sql on semicolons (being careful not to split inside function bodies, but schema.sql doesn't have any)
    ...
    with engine.begin() as conn:
        ...
        # Load activity_clean
        # Drop helper boolean columns if we don't want them in DB or keep them.
        # Our schema.sql has duration columns, is_repetitive, etc.
        # Let's align dataframe columns with schema table:
        act_columns = [
